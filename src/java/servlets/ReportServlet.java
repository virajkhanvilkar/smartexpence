...
//
//        try (Connection conn = DBConnection.getConnection()) {
//            // ✅ Step 1: Fetch available months dynamically from expenses table
//            PreparedStatement psMonths = conn.prepareStatement(
//                "SELECT DISTINCT MONTH(date) AS m, YEAR(date) AS y " +
//                "FROM expenses WHERE user_id=? ORDER BY y DESC, m DESC"
//            );
//            psMonths.setInt(1, userId);
//            ResultSet rsMonths = psMonths.executeQuery();
//
...
//
//            while (rsMonths.next()) {
//                int m = rsMonths.getInt("m");
//                int y = rsMonths.getInt("y");
//                availableMonths.add(monthNames[m] + " " + y);
//            }
//
//            request.setAttribute("availableMonths", availableMonths);
//
//            // ✅ Step 2: If a month was selected, generate report data
